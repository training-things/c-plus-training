...
//friend class BaseTime<typename T>;
template<typename T>
...
inline basic_stopwatch<T>::basic_stopwatch(bool start_now)
        : m_activity("Stopwatch"), m_lap(0), m_log(std::cout) {
    if (start_now)
        Start();
}
...
inline basic_stopwatch<T>::basic_stopwatch(char const *activity, bool start_now)
        : m_activity(activity && activity[0] ? activity : nullptr), m_lap(0), m_log(std::cout) {
    if (start_now) {
        if (m_activity)
            Start();
        else
...
inline basic_stopwatch<T>::basic_stopwatch(std::ostream &log, char const *activity, bool start_now)
        : m_activity(activity && activity[0] ? activity : nullptr), m_lap(0), m_log(log) {
    if (start_now) {
        if (m_activity)
            Start();
        else
